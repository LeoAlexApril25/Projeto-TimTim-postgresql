...
const getMonthlySummary = async (req, res) => {
    try {
        ...
        const [[goal]] = await db.query(`
            ...
            FROM goals WHERE month_year = ? LIMIT 1
        `, [month_year]);
        ...
        const comparativo = prevFaturamento > 0
            ? Number(((faturamento - prevFaturamento) / prevFaturamento * 100).toFixed(1))
            : null;
        const orcamento = Number(goal?.target_amount || 0);
        const percentGastos = orcamento > 0 ? Number((gastos / orcamento * 100).toFixed(1)) : null;

        res.json({ month_year, faturamento_mensal: faturamento, comparativo_mes_anterior: comparativo, gastos_atuais: gastos, orcamento_planejado: orcamento, percent_gastos: percentGastos, lucro_estimado: lucroEstimado });
    } catch (err) {
...
const getPreviousMonthSummary = async (req, res) => {
    try {
        ...
        const faturamentoLiquido = faturamentoBruto - Number(cmv.total);

        res.json({ month_year: prev, faturamento_bruto: faturamentoBruto, faturamento_liquido: faturamentoLiquido, despesas });
    } catch (err) {
